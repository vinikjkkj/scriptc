...
Measured 2026-08-30 on win32 x86_64, `zig 0.16.0` (`<zapo-work>\tools\zig`),
...
`<blocks>\wamfix-lab\app`; provenance source cache copied to
`<blocks>\wamfix-lab\prov`. `<zapo-work>` was read only apart from this file.
...
Paths, all under `<blocks>\` and nothing at `G:\` top level:
`<blocks>\wamfix` (worktree), `<blocks>\wamfix-lab\` (`app/`, `bin/` with a
README, `prov/`, `provmap-BEFORE/`, `provmap-AFTER/`, `sites.mjs`,
`provmap.mjs`, `bo.sh`, `gate.sh`, `env.sh`), `<blocks>\wamfix-tmp` (TMP,
