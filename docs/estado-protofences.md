...
Worktree `<blocks>\protofences` (branch `block/protofences`, from main `2b05d613`).
Lab `<blocks>\protofences-lab`, tmp `<blocks>\protofences-tmp`, env in `env.sh`.
...
Everything below was RUN under `node v25.9.0` (`<home>/AppData/Local/nvm/v25.9.0`),
...
From `<blocks>\wamfences-lab\bin\wam-entry2.BASE.c` (141,679,052 bytes, the
...
`<blocks>\sizespeed-lab\builds\base\messaging.bench.c` — 130,235,288 bytes,
...
## The floor, re-verified on this host today
    ...
    <blocks>\twininit-lab> ./bin/wam-entry2-be.c.exe | diff - bin/wam-entry2-be.node.out
    exit=0, no diff — 17 lines byte-identical, 15 `ok` assertion lines.

Nothing in `<blocks>\twininit-lab` was written to. My copy of the driver and
