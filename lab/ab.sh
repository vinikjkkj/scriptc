#!/usr/bin/env bash
# BASE-vs-BRANCH over the whole program set, in the SAME lane.
#   bash lab/ab.sh base     # stashes the tree, rebuilds dist, scores, restores
#   bash lab/ab.sh branch
# The score file is runs/ab-<side>.txt; compare-ab.sh diffs the two and
# reports "N WRONG->MATCH, M MATCH->WRONG". M must be zero.
set -u
side=$1
. ${BLOCKS_ROOT:-<blocks>}/twobyte/lab/env.sh
cd ${BLOCKS_ROOT:-<blocks>}/twobyte || exit 1
BASE=${BASE:-70e1fe48}
if [ "$side" = base ]; then
  git checkout "$BASE" -- packages/compiler/src packages/runtime/src || exit 1
fi
(cd packages/compiler && node node_modules/typescript5/bin/tsc -p tsconfig.json) >/dev/null 2>&1
(cd packages/cli && node ../../node_modules/typescript/bin/tsc -p tsconfig.json) >/dev/null 2>&1
OUT=${BLOCKS_ROOT:-<blocks>}/twobyte-lab/runs/progs-$side bash lab/run.sh lab/progs/*.ts tests/corpus/7327-*.ts tests/corpus/7328-*.ts tests/corpus/7329-*.ts \
  > ${BLOCKS_ROOT:-<blocks>}/twobyte-lab/runs/ab-$side.txt 2>&1
if [ "$side" = base ]; then
  git checkout HEAD -- packages/compiler/src packages/runtime/src || echo "RESTORE FAILED -- fix by hand"
  (cd packages/compiler && node node_modules/typescript5/bin/tsc -p tsconfig.json) >/dev/null 2>&1
  (cd packages/cli && node ../../node_modules/typescript/bin/tsc -p tsconfig.json) >/dev/null 2>&1
fi
wc -l < ${BLOCKS_ROOT:-<blocks>}/twobyte-lab/runs/ab-$side.txt
