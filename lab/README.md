...
| `progs/` | the 18 differential programs. `t01`–`t18`; the ones worth keeping permanently were promoted into `tests/corpus/7324`–`7326`. |
| `bin/` | every compiled probe, both backends, kept on purpose (git-ignored, not deleted). Run one directly; its Node oracle output is beside it in `<blocks>/twobyte-lab/runs/progs/<name>.node.txt`. |

Captured output, the voip re-measurement and the gate log live outside the
worktree in `<blocks>/twobyte-lab/runs/` so a rebuild never sweeps them.
...
The voip re-measurement uses `<blocks>/twobyte-lab/sites.mjs` (a copy of
`tests/perf/pkgstatus/harness/sites.mjs`) against a copy of pkgstatus's lab
app at `<blocks>/twobyte-lab/app`:

```sh
node <blocks>/twobyte-lab/sites.mjs \
     <blocks>/twobyte-lab/app/domprobe/voip-entry.ts out.json --provenance-sources
```
...
`<blocks>/twobyte-lab/app/probe-mlow/mlow-codec-ctl.ts` is the probe with
