...
The lab lives at `<blocks>\voipfix-lab` (the app is a copy of the pkgstatus
block's, which survived its worktree). Binaries are in
`<blocks>\voipfix-lab\bin` and are NOT deleted — the user runs them.
...
## Environment
    ...
    export TMP='<blocks>\voipfix-tmp' TEMP=... TMPDIR=...   # OUTSIDE the worktree
    export SCRIPTC_CC=zigcc SCRIPTC_TEST_CC='zig cc' SCRIPTC_TARGET=x86_64-windows-gnu

`<blocks>\voipfix-lab\env.sh` is the sourced form. `TMP` is outside the
