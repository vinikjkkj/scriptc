...
//
// r01-r05 are the rows that fail to build on main. r06-r08 are controls: the
// one-argument agreement that already lowered, the same agreement reached
// both ways, and a promisified binding that is only ever bound.
...
async function main(): Promise<void> {
    ...
    const ab = { privateKey: alice.privateKey, publicKey: bob.publicKey }
    const ba = { privateKey: bob.privateKey, publicKey: alice.publicKey }
    ...
    // r02 — the DECLARED overload (@types/node's second one, returning void),
    // with a real (err, secret) callback: err is null and the secret is the
    // same 32 bytes the one-argument form computes.
    let seenErr = 'not-called'
    let seenHex = ''
    diffieHellman(ab, (err, secret) => {
