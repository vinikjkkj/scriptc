...
describe(`npm-static pilots${sanitize ? " (sanitized)" : ""}`, () => {
  ...
  // protobufjs static-module's ONEOF ACCESSOR family. This was pinned in
  // REFUSAL ORDER at seven runtime fences across four families, because
  // oneofish did not compile and the point was WHICH construct refused
  // first: `Msg.prototype.field = null` (2, assignment to non-variables),
  // `Object.hasOwnProperty.call(m, k)` (2), `new Payload(p)` (1), and the
  // two `Object.defineProperty` oneof accessors whose hint named the
  // RECEIVER — a prototype object is not a value — rather than the
  // descriptor. A census of the QR-path closure of a real generated
  // protobuf bundle counted 45 defineProperty refusals, 44 of them this
  // shape, and the note said: when any of the four lands, this pin fails
  // and the next reader gets a corrected picture instead of the stale 45.
  //
  // All four have landed. The idiom now compiles with NOTHING left over —
  // no build diagnostic, no runtime fence, no island statement, no skipped
  // function — so the pin inverts: it is now a regression tripwire in the
  // other direction, and a fence coming back fails it with the fence's own
  // message rather than a bare count. `runtimeFences` is absent (not empty)
  // when there are none: index.ts omits the key, so `?? []` is what makes
  // "no fences" and "no such field" read alike, and the statement stats
  // below are the independent check that this is really zero.
  test("the protobufjs oneof-accessor idiom compiles with nothing left over", () => {
    ...
    // Not a restatement of the line above: these come from the lowerer's
    // own counters, so a fence that stopped being REPORTED but still cost
    // a statement would show up here.
    expect(coverage.stats.statementsFailed).toBe(0);
    expect(coverage.stats.statementsIsland).toBe(0);
    expect(coverage.stats.functionsSkipped).toBe(0);
    expect(coverage.stats.statementsTotal).toBeGreaterThan(20);
  }, 120_000);
