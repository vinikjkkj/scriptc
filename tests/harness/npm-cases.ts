...
export function npmCases(fixturesRoot: string): NpmCase[] {
  return [
    ...globSync(join(fixturesRoot, "npm/cases/*/main.ts"))
      ...
      // 4031-4062 are the --npm-static [[Get]] family. 4032 and 4062 are
      // PRICE LISTS (the `-on-purpose` convention): programs that
      // deliberately do NOT match Node today, pinned in
      // npm-static.test.ts so the refusal has a price tag. 4031 and 4061
      ...
      .filter((entry) => !/\/(246[5-9]|255[67]|(403[12]|406[12]))-[^/]+\/main\.ts$/.test(entry))
