...
// MessageSend.
import { WaWamCoordinator } from './wam/index.js'

import { resolveWamGlobals } from './wam/globals.js'
import { resolveWamEnumValue, resolveWamEventFields, wamValueKind } from './wam/registry.js'
...
// wamPlugin is NOT called here. Its declaration carries SC2008 (the
// defineWaClientPlugin intersection), and calling it makes every use
// inherit that blocker -- measured in wam-entry.ts, which does call it.
// plugin.ts is still in the program: index.ts re-exports it.
...
console.log(fails === 0 ? 'WAM-ENTRY2: ALL PASS' : 'WAM-ENTRY2: ' + fails + ' FAILURES')
