...
Block `pkgstatus2`, branch `block/pkgstatus2`, worktree `<blocks>\pkgstatus2`,
lab `<blocks>\pkgstatus2-lab`. Base main **`7417b09f`** (the previous survey,
`<blocks>\pkgstatus-lab\estado-pkgstatus.md`, was measured on `16705f5c` plus
...
`<blocks>\pkgstatus-lab` was **not** modified or deleted.
...
**Paths**, all under `<blocks>\pkgstatus2-lab\` and `<blocks>\pkgstatus2\`,
nothing at `G:\` top level: `app/` (a copy of the previous block's lab app —
`<blocks>\pkgstatus-lab` was neither modified nor deleted), `bin/` (every
