// store-postgres/cleanup.ts, driven WITHOUT start(): the one refusal in the
// module ('unref' in this.timer, cleanup.ts:59) is inside start(), which this
// entry never reaches, so it cannot fail the build.
import { PgCleanupPoller } from '../pkgs/store-postgres/cleanup'

function bad(ms: number): string {
    try {
        const p = new PgCleanupPoller({ intervalMs: ms })
...
async function main(): Promise<void> {
    ...
    const q = new PgCleanupPoller({ intervalMs: 25 })
