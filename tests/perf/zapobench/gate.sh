#!/bin/bash
set -u
. ${BLOCKS_ROOT:-<blocks>}/zapobench/lab/env.sh
# THE ORACLE IS NOT process.execPath: the harness spawns literal "node",
# resolved from PATH. PATH node is v22.18.0; the oracle is v25.9.0.
# NOTE: PATH is colon-separated, so a Windows-spelled root splits into two entries --
# the drive letter becomes an entry of its own and the rest of the path is lost.
# *_ROOT_POSIX holds the MSYS spelling: a different VALUE, not a restyling of one.
# Do not collapse the pair back into a single variable.
export PATH="${HOME_ROOT_POSIX:-<home>}/AppData/Local/nvm/v25.9.0:$PATH"
export SCRIPTC_CACHE_DIR="${BLOCKS_ROOT:-<blocks>}\zapobench\cache\gate"
export SCRIPTC_PROVENANCE_CACHE="${BLOCKS_ROOT:-<blocks>}\zapobench\cache\gate\prov"
export ZIG_GLOBAL_CACHE_DIR="${BLOCKS_ROOT:-<blocks>}\zapobench\zig-gate"
export ZIG_LOCAL_CACHE_DIR="${BLOCKS_ROOT:-<blocks>}\zapobench\zig-gate"
export SCRIPTC_TEST_WORKERS=2
mkdir -p ${BLOCKS_ROOT:-<blocks>}/zapobench/cache/gate/prov ${BLOCKS_ROOT:-<blocks>}/zapobench/zig-gate
cd ${BLOCKS_ROOT:-<blocks>}/zapobench/wt || exit 1
"$NODE25" node_modules/vitest/vitest.mjs run \
  tests/harness packages/compiler/test packages/runtime/test \
  --exclude '**/coverage.test.ts' --exclude '**/differential.test.ts' \
  --exclude '**/llvm-differential.test.ts' \
  > ${ZAPO_WORK_ROOT:-<zapo-work>}/zapobench-artifacts/raw/gate.log 2>&1
VITEST_EXIT=$?
echo "VITEST_EXIT=$VITEST_EXIT" | tee -a ${ZAPO_WORK_ROOT:-<zapo-work>}/zapobench-artifacts/raw/gate.log
