...
| `patch-drv.mjs` | turns `<zapo-work>\...\drv.mjs` into a measuring driver by four anchored edits. Every anchor is checked; a moved anchor is a hard error, never a silent no-op. |
...
binaries in `<zapo-work>\zapobench-artifacts\` with its own README. The short
version: zig 0.16.0 from `<zapo-work>\tools\zig` must come first on `PATH`,
