...
* **Backend: this program builds on the LLVM backend, and the memory
  figures do not depend on that.** An earlier revision of this note said
  the service was “a C-backend build by construction”, reasoning that
  `WeakMap` has no LLVM lowering (`LlvmUnsupportedError("weakmap:new")`)
  and that zapo uses `WeakMap`. Rebuilt 2026-09-07, that is not what
  happens: the emitted IR is headed `Generated by scriptc (LLVM backend)`,
  contains no `weakmap` symbol at all, and the build succeeds with 67
  advisories and no demotion. **Read the lane off the artifact, not off the
  reasoning** — the header of the emitted `.ll` is the evidence, and the
  build does not otherwise announce its choice.

  The backend-independence this paragraph used to assert is now *measured*
  rather than argued. The same workload driven through an LLVM build
  reproduces the C-backend record to within the A/A floor: settled
  98.18 and 97.26 MiB against the recorded 98.37 MiB in the ~211 peak mode,
  and the budget arm at 126.97 MiB against the recorded 127.00 MiB in the
  ~218 mode. The cycle arena is in the runtime, which both backends link,
  and the measurement now says so instead of the argument.
