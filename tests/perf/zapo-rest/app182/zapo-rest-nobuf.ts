...
async function route(method: string, path: string, p: Bag, sess: Session): Promise<unknown> {
  ...
  if (path === "/message/downloadBytes") {
    ...
    if (src === undefined) {
      throw new Error(`no retained message with seq ${seq} (take 'seq' from the websocket stream — GET /messages is 410 in this build; the typed message array holds the last ${MSG_KEEP} INCOMING messages)`);
    }
