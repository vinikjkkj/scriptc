...
out.push("");
out.push("## Addressing a session");
out.push("");
out.push("The service runs **N WhatsApp sessions in one process over one SQLite file**.");
out.push("Every route below except the five under `/sessions` is addressed to exactly one");
out.push("session, and the session is named in the **path**:");
out.push("");
out.push("```");
out.push("  /s/<sessionId>/<route>        e.g.  GET /s/alice/store/threads");
out.push("  /<route>                      the same route on ZAPO_SESSION (default: \"default\")");
out.push("```");
out.push("");
out.push("A `<sessionId>` is 1-64 characters from `A-Za-z0-9` and `. _ -`. It is the SAME");
out.push("string the store writes into the `session_id` column of all 21 domain tables, so");
out.push("the identity a caller uses and the identity the rows carry are one value.");
out.push("");
out.push("The `/sessions` routes are service-level and are never prefixed. Routes under a");
out.push("session id are listed once below, without the prefix.");
out.push("");
out.push("| status | meaning |");
out.push("|---|---|");
out.push("| 200 | the call ran |");
out.push("| 400 | a required parameter is missing, the body is not JSON, or the session id is malformed |");
out.push("| 401 | `ZAPO_REST_TOKEN` is set and the `x-api-key` header does not match |");
out.push("| 404 | no such route, or no such session |");
...
out.push("");
const sessionRoutes = [...handlers.keys()].filter((k) => !k.startsWith("/sessions")).length;
const registryRoutes = handlers.size - sessionRoutes;
out.push(`Routes in this build: **${handlers.size}** — ${registryRoutes} service-level (\`/sessions...\`) plus ${sessionRoutes} addressed to a session, covering **${cov.impl} of ${cov.impl + cov.unimpl}** public members of zapo's client surface.`);
out.push("");
