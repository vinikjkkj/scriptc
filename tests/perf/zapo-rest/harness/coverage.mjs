...
const surface = readFileSync("<blocks>/restapi/lab/surface.txt", "utf8");
const routes = new Set(
  readFileSync("<blocks>/restapi/lab/routes.txt", "utf8").split("\n").map((s) => s.trim()).filter(Boolean),
);
...
writeFileSync("<blocks>/restapi/lab/coverage.json", JSON.stringify({ impl, unimpl, rows, unimplemented }, null, 1));
