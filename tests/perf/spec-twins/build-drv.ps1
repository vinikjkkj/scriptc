...
$entry = "$(if ($env:BLOCKS_ROOT) { $env:BLOCKS_ROOT } else { '<blocks>' })\pkgstatus3-lab\napp\drivers\$drv.ts"
$dir   = "$(if ($env:BLOCKS_ROOT) { $env:BLOCKS_ROOT } else { '<blocks>' })\mongoasync\out\$tag"
New-Item -ItemType Directory -Force -Path $dir | Out-Null
$out   = "$dir\$drv.exe"
$log   = "$(if ($env:BLOCKS_ROOT) { $env:BLOCKS_ROOT } else { '<blocks>' })\mongoasync\out\$tag.log"
...
node "$(if ($env:BLOCKS_ROOT) { $env:BLOCKS_ROOT } else { '<blocks>' })\mongoasync\work\packages\cli\dist\main.js" build $entry -o $out --provenance-sources *>&1 | Out-File -Append -Encoding utf8 $log
...
if (Test-Path $out) {
  ...
  $n25 = "$(if ($env:HOME_ROOT) { $env:HOME_ROOT } else { '<home>' })\AppData\Local\nvm\v25.9.0\node.exe"
  & $n25 "$(if ($env:BLOCKS_ROOT) { $env:BLOCKS_ROOT } else { '<blocks>' })\mongoasync\work\node_modules\tsx\dist\cli.mjs" $entry *>&1 | Out-File -Encoding utf8 "$dir\$drv.node.out"
