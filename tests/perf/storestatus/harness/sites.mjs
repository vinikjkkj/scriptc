...
// The guard runs FIRST, for effect: an unpinned SCRIPTC_PROVENANCE_CACHE makes
// resolveProvenanceSources() extract attested trees under homedir(), i.e. the
// user C: drive. That has happened here before -- 254 files, 10.9 MB -- from a
// bare `node checkmap.mjs`. There is no WT default on purpose: a wrong default
// silently measures another block's compiler.
import "./guard.mjs";

const WT = process.env.WT;
if (!WT) { console.error("REFUSING: WT is unset -- source harness/env.sh"); process.exit(2); }
