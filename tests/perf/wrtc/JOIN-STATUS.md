...
Base main `acdd8b96`. Worktree `<blocks>\wrtcjoin`, lab `<blocks>\wrtcjoin-lab`,
tmp `<blocks>\wrtcjoin-tmp`. Toolchain zig 0.16.0, build node v22.18.0,
...
retransmits across the suite. `<blocks>\wrtcjoin-lab\joined-base.out`.
...
- The mbedtls archive (1,228,430 bytes) was copied from
  `<repo>\packages\runtime\vendor\.cache\mbedtls-3.6.7-plain-x86_64-windows-gnu`
...
point — `<repo>`, pid 2928, three tinypool workers at 46-72 s CPU each,
