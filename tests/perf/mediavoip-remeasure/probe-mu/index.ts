...
export function createMediaProcessor(options?: WaMediaProcessorOptions): WaMediaProcessor {
    ...
    return {
        ...
        async detectMimetype(input: string | Uint8Array, ctx?: WaMediaProcessorCallContext) {
            try {
                // PROBE CHANGE, the only one in this copy of media-utils.
                //
                // `file-type`'s "." export is
                //   { node: { import: index.js }, default: { import: core.js } }
                // and it has NO "./node" subpath. scriptc forces
                // moduleResolution: Bundler (frontend/program.ts:82-93), the
                // "node" condition is therefore never applied, the import
                // resolves to core.js, and `fileTypeFromFile` -- which exists
                // only in index.js -- is not there. There is no source-level
                // workaround: the member cannot be reached at all under
                // Bundler resolution.
                //
                // So this probe drops the string arm rather than fixing it. It
                // answers only "what is behind that one line", and the number
                // it produces is not a number the real module can reach today.
                const fileType = await import('file-type')
                const result =
                    typeof input === 'string'
                        ? await fileType.fileTypeFromBuffer(new Uint8Array(0))
