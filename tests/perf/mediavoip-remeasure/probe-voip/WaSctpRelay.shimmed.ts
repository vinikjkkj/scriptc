...
// PROBE SHIM, and the ONLY change in this copy of the voip tree.
//
// scriptc forces `lib: ["lib.es2025.d.ts"]` (frontend/program.ts:82), so the
// global DOM types do not exist, and `@roamhq/wrtc`'s .d.ts re-exports the
// global `RTCPeerConnection`/`RTCDataChannel` rather than declaring them.
// These two lines are what fails preflight for seven of voip's eight entries.
//
// These declarations are NOT the DOM lib -- they are the minimum that lets
// preflight past line 30, and they exist to answer one question: what does
// voip's entry hide behind those two names? Any number measured through this
// shim is a LOWER bound on what the real lib would report, never an upper one.
declare global {
    interface RTCPeerConnection {
        readonly connectionState: string
        readonly iceConnectionState: string
        createDataChannel(label: string, init?: unknown): RTCDataChannel
        ...
        readonly localDescription: unknown
    ...
    const RTCPeerConnection: { new (config?: unknown): RTCPeerConnection }
    const RTCDataChannel: { new (): RTCDataChannel }
}
