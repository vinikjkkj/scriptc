...
Paths, all under `<blocks>\pkgstatus-lab\`, nothing at `G:\` top level:
`app/` (lab app, 147 modules + drivers + probes), `bin/` (**every binary, kept,
with a README**), `sites-default/`, `sites-prov/`, `prov/`, `prov-fix/`.
`<zapo-work>` was read only, apart from this report. The harness, both
...
9. **"`<blocks>\stores\lab\pg-client.patch`"** — still does not exist; the
   whole `<blocks>\stores` tree is gone.
