...
tmp at `<blocks>\wamvoip-tmp` for that reason — this block's worktree is
`<blocks>\wamvoip` itself, where other blocks keep theirs at
`<blocks>\<name>\wt` with `tmp` as a sibling.
