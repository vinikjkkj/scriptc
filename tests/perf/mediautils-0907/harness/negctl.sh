...
L=${BLOCKS_ROOT:-<blocks>}/mediautils-work
{
  ...
  WT=${BLOCKS_ROOT:-<blocks>}/mediautils node "$L/sites.mjs" tests/fixtures/node-types/child-stdio.ts "$L/sites/fixture-child-stdio-before.json" 2>&1
  echo "--- restoring ---"
  git stash pop 2>&1
  pwsh -NoProfile -File "$L/build.ps1"; tail -1 "$L/build1.log"
  echo "--- AFTER (changes restored) ---"
  WT=${BLOCKS_ROOT:-<blocks>}/mediautils node "$L/sites.mjs" tests/fixtures/node-types/child-stdio.ts "$L/sites/fixture-child-stdio-after.json" 2>&1
  echo "--- git status ---"
  git status --short
