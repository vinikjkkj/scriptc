...
/* THE ONE CHANGE: the module is imported STATICALLY instead of through
 * `cachedArgo = await import('argo-codec')`. Everything else -- every line of
 * the decoder -- is zapo's, byte for byte. If this decodes and the dynamic one
 * does not, the whole gap is the namespace --npm-static builds for import().
 */
import { Reader as ArgoReader } from 'argo-codec'

async function loadArgo(): Promise<boolean> {
    return true
}

export async function isMexArgoDecoderAvailable(): Promise<boolean> {
    return await loadArgo()
}
...
export async function decodeMexArgoResponse(bytes: Uint8Array): Promise<WaMexArgoResponse> {
    ...
    if (!inline) {
        while (r.pos < r.end) {
            const len = r.label()
            segments.push(new ArgoReader(r.bytes(len)))
        }
