 ...
 */
import { decodeMexArgoResponse, isMexArgoDecoderAvailable } from './argo-decoder-static'
...
export async function main(): Promise<void> {
    ...
    console.log('[decode-static] bytes=' + String(bytes.length))
    ...
    try {
        ...
        console.log('[decode-static] isMexArgoDecoderAvailable = ' + String(avail))
    } catch (err) {
        console.log('[decode-static] availability THREW: ' + (err as Error).message)
    }
    ...
    try {
        ...
        if (tl === null || tl === undefined) {
            console.log('[decode-static] DECODED but the timelock field is absent')
            return
        }
        console.log(
            '[decode-static] OK isActive=' +
                String(tl['is_active']) +
                ' enforcementType=' +
        ...
        console.log('[decode-static] DECODE THREW: ' + (err as Error).message)
    }
}
void main()
