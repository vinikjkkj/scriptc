 ...
 * scr_runtime.h. Everything below is the four operations a compiled
 * instance can ask of it; there is no fifth, and that is what makes the
...
ScrDyn *scr_cls_props_get(const ScrDyn *tbl, const ScrStr *key) {
  ...
  if (scr_hid_is_data(ent)) return scr_dyn_retain(scr_hid_value(ent));
  ScrDyn *g = scr_hid_getter(ent);
...
bool scr_cls_props_define(ScrDyn *tbl, const ScrStr *key, ScrDyn *desc,
  ...
  if (cur != NULL && cur->v.arr.len >= 5) {
    ...
    curConf = scr_hid_configurable(cur);
    curWrite = scr_hid_is_data(cur) && scr_hid_writable(cur);
    if (!curConf) {
      ScrJsonBuf b;
      scr_jb_init(&b);
