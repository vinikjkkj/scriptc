...
static const char web_prelude[] =
    ...
    "      }\n"
    "      /* An INHERITED body counts too. `new Request(postReq, {method:'GET'})`\n"
    "       * carries the SOURCE request's bytes, and the check is on the\n"
    "       * request's body AFTER init is applied, not on init.body alone --\n"
    "       * which is the ONLY thing the branch above can see. Node throws\n"
    "       * here; the island accepted it and then sent a GET carrying a body.\n"
    "       * fetch's glue reaches this same constructor (`new g.Request(base,\n"
    "       * reqInit)` in scr_fetch.c and scr_fetch_curl.c), so\n"
    "       * `fetch(postReq, {method:'GET'})` was the same wrong answer.\n"
    "       * `body: null` in init does NOT clear an inherited body, and an\n"
    "       * EMPTY-string body is still a body -- the test is `!== null`, never\n"
    "       * truthiness. */\n"
