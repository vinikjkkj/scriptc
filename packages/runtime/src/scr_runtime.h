 ...
 * Require stack line. The builtins argument is Node's own builtinModules,
 * joined and delimited the same way, and decides a node: specifier; the
 * scope argument carries the requiring file's package scope for a #
 * import: "" cannot enumerate, "-" has no imports field, and
