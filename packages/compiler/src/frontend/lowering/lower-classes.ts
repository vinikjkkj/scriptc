...
const WEAK_COLLECTION_HINTS = {
  WeakMap:
    ...
    "route the value can die by — Uint8Array, and ARRAYS of any element type. A bare " +
    "'object' key is the dyn (TypeScript's 'object' is a top type over every " +
    "non-primitive), and a dyn box is a boundary artifact rather than the value, so an " +
    "address-keyed table over boxes would miss every lookup: name the concrete key type " +
    "instead, which for an array-keyed cache is the array type itself. A RECORD key is " +
    "refused because it width-coerces, so the entry would be keyed on the copy's address " +
    "and not the caller's; a CLASS INSTANCE because an acyclic class is allocated without " +
    "a cycle header and never reaches the collector-side hook. Values must be a reference " +
    "type (a scalar has no pointer for the table to hold). A strong Map accepts the " +
    "refused keys today, but it RETAINS them — for a cache keyed on a long-lived object " +
