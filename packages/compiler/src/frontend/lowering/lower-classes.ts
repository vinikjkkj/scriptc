...
const WEAK_COLLECTION_HINTS = {
  WeakMap:
    "WeakMap is supported, but only for keys whose death this runtime observes on every " +
    "route the value can die by — Uint8Array, ARRAYS of any element type, and a bare " +
    "'object' (TypeScript's 'object' is a top type over every non-primitive, which lowers " +
    "to the dyn). An 'object'-keyed table is keyed on each value's PAYLOAD rather than on " +
    "its dyn box — the box is a boundary artifact and two boxes of one value are the same " +
    "JS value — so it admits a Uint8Array, an array, or a value already typed 'unknown', " +
    "and set() refuses the rest by name at run time. A RECORD key is refused because it " +
    "width-coerces, so the entry would be keyed on the copy's address and not the " +
    "caller's; a CLASS INSTANCE because an acyclic class is allocated without a cycle " +
    "header and never reaches the collector-side hook. Values must be a reference type " +
    "(a scalar has no pointer for the table to hold) — that is the one this program is " +
    "most likely to have hit if its key type is fine. A strong Map accepts the refused " +
    "keys today, but it RETAINS them — for a cache keyed on a long-lived object that is " +
    "unbounded growth",
  WeakSet:
